...
from adapt_algorithm import PseudoLabel,T3A,BN,ERM,Tent,TSD,Energy,SAR,SAM,EATA,TIPI,S-FRET,G-FRET
...
if __name__ == '__main__':    
    ...
    if args.adapt_alg=='Tent':
        ...
        params,_ = collect_params(algorithm)
        optimizer = torch.optim.Adam(params,lr=args.lr) 
        adapt_model = S-FRET(algorithm,optimizer)
    elif args.adapt_alg=='G-FRET':
        #optimizer = torch.optim.Adam(algorithm.parameters(),lr=args.lr)
        algorithm = configure_model(algorithm)
        params,_ = collect_params(algorithm)
        optimizer = torch.optim.Adam(params,lr=args.lr)   
        adapt_model = G-FRET(algorithm,optimizer,lam_GFRET=args.lam_GFRET,filter_K=args.filter_K
