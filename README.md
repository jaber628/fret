...
python unsupervise_adapt_corrupted.py --dataset CIFAR-10 \
                                      ...
                                      --net resnet18
```
Change `--adapt_alg GFRET` to use different methods of test time adaptation, e.g. `FRET`, `TSD`, `BN`, `Tent`.  
`--pretrain_dir` denotes the path of source model, e.g. `./train_outputs/model.pkl`.  
For GFRET, we have set default parameters in our code. For better results, you might consider adjusting the parameters `--lam_GFRET`,`--filter_K`, and `--GFRET_K`. For guidance on selecting hyperparameters, please refer to our paper.
## Tested Environment
