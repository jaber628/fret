...
from adapt_algorithm import PseudoLabel,T3A,BN,ERM,Tent,TSD,Energy,SAR,SAM,EATA,TIPI,FRET,GFRET
...
def get_args():
    ...
    parser.add_argument('--lam_GFRET', type=float,
                        default=100, help="tradeoff hyperparameter used in GFRET")
    parser.add_argument('--GFRET_K', type=float,
                        default=0.95, help="filter hyperparameter used in GFRET")
    parser.add_argument('--label_consistance', type=bool,
                        default=True, help="label_consistance hyperparameter used in GFRET")
    ...
    parser.add_argument('--filter_K',type=int,default=100,help='M in T3A/TSD/GFRET, \in [1,5,20,50,100,200,300,-1],-1 denotes no selectiion')
...
if __name__ == '__main__':    
    ...
    if args.adapt_alg=='Tent':
        ...
        params,_ = collect_params(algorithm)
        optimizer = torch.optim.Adam(params,lr=args.lr) 
        adapt_model = FRET(algorithm,optimizer)
    elif args.adapt_alg=='GFRET':
        #optimizer = torch.optim.Adam(algorithm.parameters(),lr=args.lr)
        algorithm = configure_model(algorithm)
        params,_ = collect_params(algorithm)
        optimizer = torch.optim.Adam(params,lr=args.lr)   
